# FastMDAnalysis/src/fastmdanalysis/analysis/q_value.py
...
Outputs
-------
- q_value.dat           : (N,) array of Q values per frame
- q_value_metadata.json : JSON file with native contacts count and parameters
- q_value.png           : line plot of Q vs frame with metadata annotation
...
class QAnalysis(BaseAnalysis):
    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        -------
        dict
            {"q_value": (N,) array of Q values per frame}
        """
        try:
            ...
            self.results = {"q_value": self.data}

            # Save data
            logger.info("Saving Q-value data...")
            self._save_data(self.data, "q_value", header="q_value", fmt="%.6f")
    ...
    def _save_metadata(self):
        ...
        metadata_path = self.outdir / "q_value_metadata.json"
    ...
    def plot(self, data: Optional[np.ndarray] = None, **kwargs):
        ...
        out = self._save_plot(fig, "q_value")
        plt.close(fig)
