...
# Show usage
usage() {
    echo "  FastMDAnalysis Release Manager"
    echo ""
    echo "  USAGE:"
    echo "  scripts/release.sh <version> [release_notes]    Interactive release process"
    echo "  scripts/release.sh quick <version>              Quick release (minimal prompts)"
    echo "  scripts/release.sh next                         Auto-increment patch version"
    echo "  scripts/release.sh current                      Show current version"
    echo "  scripts/release.sh help                         Show this help message"
    echo ""
    echo "  EXAMPLES:"
    echo "  scripts/release.sh 0.0.3"
    echo "  scripts/release.sh 1.0.0 \"Major release with new features\""
    echo "  scripts/release.sh quick 0.0.4"
    echo "  scripts/release.sh next"
    echo ""
    echo "  SEMANTIC VERSIONING:"
    echo "  MAJOR.MINOR.PATCH (e.g., 1.2.3)"
    echo "  - MAJOR: Breaking changes"
    echo "  - MINOR: New features (backwards compatible)"
    echo "  - PATCH: Bug fixes"
    echo ""
    echo "  AUTO-RELEASE:"
    echo "  Pushing the tag triggers GitHub Actions to:"
    echo "  - Run tests • Build package • Upload to PyPI • Create GitHub release"
}
...
# Check if we're in the scripts directory and adjust if needed
check_script_location() {
    local script_dir=$(dirname "$0")
    if [[ "$script_dir" != "." ]]; then
        print_warning "Script is in $script_dir, changing to scripts directory"
        cd "$script_dir"
    fi
}
