...
.. code-block:: python
   ...
   rmsd = fastmda.rmsd(reference_frame=0, atoms="protein and name CA", output="analysis/rmsd")
   results = rmsd.run()
...
.. code-block:: bash
   ...
   fastmda rmsd -traj traj.dcd -top top.pdb --reference-frame 0 --atoms "protein and name CA" \
...
- ``rmsd_output/rmsd.dat`` — two-column text file with frame index and RMSD
   (nm).
- ``rmsd_output/rmsd_stats.dat`` — mean and standard deviation when
   ``compute_stat=True``.
